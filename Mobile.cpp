...
void Mobile::loop() {
	...
	//read socket every 30ms to reduce serial port msg
	if( this->lastRcvTime != 0 && (currTime - this->lastRcvTime) < 40){
		return;
	}
...
void Mobile::processMessage(EthernetClient* conn, uint8_t* buf, uint8_t buf_size){
	...
	if(0 == strncmp((char*)buf, "AU", 2)) {
		if(cmdata->remote->connected == REMOTE_CONN_NOT_RESOLVED) { 
			//if dns not done, just return.
		//	return;
		}
		else if(cmdata->remote->connected == REMOTE_CONN_NOT_REQUEST) {
			dropMobileClent(1);
			cmdata->remote->connect();
		}

		EepromManage::AddEepromMacToWhiteList((char*)(buf+p+1));
		//send new mac to server
		cmdata->remote->sendMsgToServer((uint8_t*)APP_AUTH_MAC, strlen(APP_AUTH_MAC));
		cmdata->remote->sendMsgToServer((unsigned char*)(buf+p+1), E2DA_WHITE_LIST_MAC_LEN);
		cmdata->remote->sendMsgToServer((uint8_t*)"\n", 1);
		conn->print("AUB\r\n");
		return;
	}
...
//		conn->print("RTB 0001,10,0\r\n");
	...
	//delete group
	if(0 == strncmp((char*)buf,"DG",2)){
		this->cmdata->com->write((char*)buf, buf_size);
		return;
	}

	//search group
	if(0 == strncmp((char*)buf,"SGP",3)){
		this->cmdata->com->write((char*)buf, buf_size);
		return;
	}

	//search group member
	if(0 == strncmp((char*)buf,"SGM",3)){
		this->cmdata->com->write((char*)buf, buf_size);
		return;
	}
...
/*f(0 == strncmp((char*)buf, "EEPROMDUMP", 10)) {
	...
	if(0 == strncmp((char*)buf, "CH", 2)) {
		char src[13];
		snprintf(src, 13, "%s%s", this->cmdata->key, (buf + p + 1));
		conn->print("FROM_YEEBOX_FOR_AUTH:");
		conn->print(EepromManage::GetHash(src));
		//conn->print(src);
		conn->print("\n");
	}
	return;
