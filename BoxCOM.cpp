...
//		this->_lastHBTime = millis();
		...
		_cmdata->remote->sendMsgToServer(_rxBuf, _rxSize);
		return;
	}
...
//		this->_lastHBTime = millis();
	...
	//Add Group
	else if(0 == strncmp((char*)_rxBuf, "AGB", 3)){
		_cmdata->mobile->sendMessageToClient(_rxBuf, _rxSize);
		return;
	}
	//Delete Group
	else if(0 == strncmp((char*)_rxBuf, "DGB", 3)){
		_cmdata->mobile->sendMessageToClient(_rxBuf, _rxSize);
		return;
	}
	//Search Group
	else if(0 == strncmp((char*)_rxBuf, "SGPB", 4)){
		_cmdata->mobile->sendMessageToClient(_rxBuf, _rxSize);
		return;
	}
	//Search Group Member
	else if(0 == strncmp((char*)_rxBuf, "SGMB", 4)){
		_cmdata->mobile->sendMessageToClient(_rxBuf, _rxSize);
		return;
	}
}
